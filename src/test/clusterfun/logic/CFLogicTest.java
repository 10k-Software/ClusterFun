...
public class CFLogicTest extends TestCase
{
    ...
    public void testProcessRequestHint_easy()
    {
        boolean giveHintMessageWasSent = false;

        MessageManager.sendMessage(new GameMessage(GameMessage.MessageType.REQUEST_HINT, DifficultyType.EASY), MessageDest.CF_LOGIC);
        ...
        ArrayList<Card> hintCards = (ArrayList<Card>)messages.get(0).getValue();

        assertTrue(giveHintMessageWasSent);
        assertEquals(2, hintCards.size());

    }

    public void testProcessRequestHint_medium()
    {
        boolean giveHintMessageWasSent = false;

        MessageManager.sendMessage(new GameMessage(GameMessage.MessageType.REQUEST_HINT, DifficultyType.MEDIUM), MessageDest.CF_LOGIC);
        ...
        assertTrue(giveHintMessageWasSent);
        assertEquals(1, hintCards.size());

    }

    public void testProcessRequestHint_hard()
    {
        boolean giveHintMessageWasSent = false;

        MessageManager.sendMessage(new GameMessage(GameMessage.MessageType.REQUEST_HINT, DifficultyType.HARD), MessageDest.CF_LOGIC);
        ...
        assertTrue(giveHintMessageWasSent);
        assertEquals(0, hintCards.size());

    }

    public void testProcessRequestHint_difficultyNotSet()
    {
        ...
        {
            assertEquals("No difficulty specified in hint request message.", e.getMessage());
        }
    }
    ...
    // Reveals bug in ticket #205
    public void testGameStartMessageReceived()
    {
        // Select a card
        MessageManager.sendMessage(new GameMessage(GameMessage.MessageType.SELECT_CARD,
            new Card("Three",  "Red",     "Squiggles",  "Empty")), MessageDest.CF_LOGIC);

        // Send a game start message, which should clear the board
        MessageManager.sendMessage(new GameMessage(GameMessage.MessageType.GAME_START, null),
            MessageDest.CF_LOGIC);

        // Add 3 new cards and see if it breaks
        try
        {
            this.logic.update(0);
        }
